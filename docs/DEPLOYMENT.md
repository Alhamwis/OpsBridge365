...
| --- | --- | --- | --- |
| H1 | **Azure subscription** | ✅ Done | "Azure for Students" on the college-managed tenant. Resource group `rg-opsbridge365` — created in `eastus`, then **recreated in `westus2`** once the allowed-regions policy refused `eastus`. See [§3](#3-requestdisallowedbyazure--your-region-may-be-forbidden-by-policy) |
